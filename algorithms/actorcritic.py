...
class ActorCriticAgent(agents.Agent):
    def __init__(self, obs_space, action_space, state_action_encoder, beta=100):
        self.name = 'Soft Actor Critic Tabular Agent'
    ...
    def update(self, train_batch, lr = 1, gamma = 0.99):
        ...
        for data in train_batch:
            ...
            V = self.model[self.state_action_encoder(s_t, a_t)] - (1/beta) * np.log(pi)
    ...
    def visualize(self, L = 5):
        for s_t in len(L * L):
            print('visualize')
            print(''.join(self.model[self.state_action_encoder(s_t, a_t)] for a_t in len(4)], '-'))

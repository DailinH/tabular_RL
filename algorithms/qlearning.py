...
# offline q learning agent without target functions.
class QLearningAgent(agents.Agent):
    def __init__(self, obs_space, action_space, state_action_encoder):
        self.name = 'ql'
    ...
    def update(self, train_batch, lr = 1, gamma = 0.99):
        ...
        for data in train_batch:
            ...
            # print("update", gamma)
            self.model[self.state_action_encoder(s_t, a_t)] += lr * (r + gamma * max([self.model[self.state_action_encoder(s_tp1, a)] for a in range(self.action_space.n)]) - self.model[self.state_action_encoder(s_t, a_t)])
        # print(self.model)
